...
CONFIG_DIR="configs/rebuild/cross_pattern_1500_500_gate1_aug"
LOG_DIR="outputs/rebuild_reproduction/logs"
WORKFLOW_DIR="outputs/rebuild_reproduction/cross_pattern_1500_500_no_rpm_lateconcat_gate1_aug_queue"
MARKER_DIR="outputs/rebuild_reproduction/session_markers"
SUMMARY_PATH="${WORKFLOW_DIR}/summary.json"
MARKER_PATH="${MARKER_DIR}/cross_pattern_1500_500_no_rpm_lateconcat_gate1_aug_queue.done"
BASE_PORT="${CROSSPAT_GATE1_AUG_BASE_PORT:-30801}"

RUN_NAMES=(
  "crosspat_train345_test1_no_rpm_lateconcat_gate1_augv2noise_lr2e5_ls005_ep35"
  "crosspat_train134_test5_no_rpm_lateconcat_gate1_augv2noise_lr2e5_ls005_ep35"
  "crosspat_train135_test4_no_rpm_lateconcat_gate1_augv2noise_lr2e5_ls005_ep35"
)
CONFIGS=(
...
config_dir = Path("configs/rebuild/cross_pattern_1500_500_gate1_aug")
...
for split_name, train_root, test_root in splits:
    run_name = f"crosspat_{split_name}_no_rpm_lateconcat_gate1_augv2noise_lr2e5_ls005_ep35"
    cfg = {
        ...
        "training": {
            ...
            "acceptance": {
                ...
                "test_root": test_root,
                "note": (
                    "Non-architecture cross-pattern retry. No RPM, fixed 50 frames, late_concat "
                    "pattern conditioning with gate init 1.0, augv2_noise training augmentation, "
                    "label smoothing 0.05, lower LR, and val_loss checkpointing."
                ),
            },
        },
        "misc_dir": {
...
payload.update(
    {
        ...
        "method": (
            "Cross-pattern no-RPM fixed-50 late_concat retry with pattern_gate_init=1.0, "
            "augv2_noise, label smoothing, and lower LR."
        ),
...
if [ -z "${WANDB_API_KEY:-}" ]; then
  echo "WANDB_API_KEY is required for W&B-backed cross-pattern gate1 augmentation training." >&2
  exit 1
fi
