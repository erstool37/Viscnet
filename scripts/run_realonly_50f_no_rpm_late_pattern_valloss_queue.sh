...
if [ -z "${WANDB_API_KEY:-}" ]; then
  echo "WANDB_API_KEY is required for W&B-backed real-only late-pattern val-loss training." >&2
  exit 1
fi
...
CONFIGS=(
  "configs/rebuild/retries/realonly_993_50f_no_rpm_pattern_lateconcat_valloss_win30sched_ep50.yaml"
  "configs/rebuild/retries/realonly_993_50f_no_rpm_pattern_lateresidual_valloss_win30sched_ep50.yaml"
)
RUN_NAMES=(
  "repro_realonly_993_50f_no_rpm_pattern_lateconcat_valloss_win30sched_ep50"
  "repro_realonly_993_50f_no_rpm_pattern_lateresidual_valloss_win30sched_ep50"
)

LOG_DIR="outputs/rebuild_reproduction/logs"
WORKFLOW_DIR="outputs/rebuild_reproduction/realonly_50f_no_rpm_late_pattern_valloss_queue"
MARKER_DIR="outputs/rebuild_reproduction/session_markers"
SUMMARY_PATH="${WORKFLOW_DIR}/summary.json"
MARKER_PATH="${MARKER_DIR}/realonly_50f_no_rpm_late_pattern_valloss_queue.done"
BASE_PORT="${REALONLY_50F_LATE_PATTERN_VALLOSS_BASE_PORT:-30561}"
...
payload.update(
    {
        ...
        "schedule_source": "configs/rebuild/retries/realonly_993_window30x21_no_rpm_ep50.yaml",
        "method": (
            "Two real-only fixed-50-frame no-RPM pattern-head reruns: late_concat and "
            "late_residual. They use val_loss checkpointing/early stopping and the "
            "epoch/LR/scheduler from repro_realonly_993_window30x21_no_rpm_ep50."
        ),
    }
)
