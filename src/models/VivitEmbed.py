...
class VivitEmbed(nn.Module):
    def __init__(
        ...
        # FC HEAD
        if class_bool:
            self.fc = nn.Sequential(nn.Linear(self.hidden_size, 192), nn.SiLU(), nn.Linear(192, visc_class))
        else:
            self.fc = nn.Sequential(nn.Linear(self.hidden_size, 192), nn.SiLU(), nn.Linear(192, 3))
