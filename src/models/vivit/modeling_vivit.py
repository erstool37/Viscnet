...
class VivitEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.rpm_embed = nn.Embedding(config.rpm_class, config.hidden_size)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.config = config

    def forward(self, pixel_values, rpm_idx):
        ...
        # add position and rpm encoding to each token
        embeddings = embeddings + self.position_embeddings + rpm_tok
...
class VivitModel(VivitPreTrainedModel):
    ...
    def forward(
        ...
        embedding_output = self.embeddings(pixel_values, rpm_idx)
