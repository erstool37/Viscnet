...
# set train path with the synthetic dataset
train_video_paths, _ = train_test_split(video_paths, test_size=TEST_SIZE, random_state=RAND_STATE)
train_para_paths, _= train_test_split(para_paths, test_size=TEST_SIZE, random_state=RAND_STATE)
...
# set validation path with the real dataset, please divide the test set in separate folders, cuz the test size is 1
...
real_val_ds = dataset_class(real_video_paths, real_para_paths, FRAME_NUM, TIME)
real_val_sampler = DistributedSampler(real_val_ds, num_replicas=world_size, rank=rank, shuffle=False)
val_dl = DataLoader(real_val_ds, batch_size=BATCH_SIZE, sampler=real_val_sampler, num_workers=NUM_WORKERS)
