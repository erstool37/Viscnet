...
class VideoDataset(Dataset):
    ...
    def _loadparameters(self, para_path):
        ...
        return torch.tensor([density, dynVisc, surfT, kinVisc], dtype=torch.float32), frame
    ...
    def embedding(self, rpm):
        mlp = nn.Sequential(
            nn.Linear(1, 128),
            nn.ReLU(),
            nn.Linear(128, 1)  # Output a single scalar or small patch
        )
