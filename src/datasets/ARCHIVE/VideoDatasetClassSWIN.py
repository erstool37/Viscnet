...
class VideoDatasetClassSWIN(Dataset):
    ...
    def _loadvideo(self, video_path):
        ...
        # Normalize each frame (T, H, W, C)
        frames_aug = [((torch.tensor(f, dtype=torch.float32) / 255.0).permute(2, 0, 1) - mean[:, None, None]) / std[:, None, None] for f in frames_aug]

        # Stack into (T, C, H, W) → permute to (C, T, H, W)
        frames_tensor = torch.stack(frames_aug).permute(1, 0, 2, 3)  # (C, T, H, W)
