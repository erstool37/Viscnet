...
def confusion_matrix(
    ...
    normalize=True,
    class_names=None,
    save_dir="src/inference/confusion_matrix",
    vmax=1.0,
):
    ...
    cm_counts = sk_cm(y_true, y_pred, labels=labels, normalize=None)
    ...
    disp = ConfusionMatrixDisplay(
        confusion_matrix=cm, display_labels=(class_names if class_names is not None else labels)
    )
    ...
    plt.close()
    with open(os.path.join(save_dir, f"{name}_metrics.json"), "w") as file:
        json.dump(
            {
                "accuracy": float(accuracy),
                "labels": [int(label) for label in labels],
                "support": [int(v) for v in row_totals],
                "per_class_accuracy": [float(v) for v in per_class_accuracy],
                "confusion_matrix_counts": cm_counts.astype(int).tolist(),
...
def plot_error_distribution(name, preds_all, tgts_all, descaler, path, save_dir="src/inference/error_plots", topk=10):
    os.makedirs(save_dir, exist_ok=True)
    ...
    plt.gcf().text(
        0.98, 0.52, infostr, ha="right", va="top", fontsize=9, bbox=dict(boxstyle="round", fc="w", ec="0.6", alpha=0.9)
    )
...
def reliability_diagram(preds_local, tgts_local, name="test", save_dir="src/inference/reliability_plots"):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f"{name}.png"), dpi=1000, bbox_inches="tight")
    plt.close()
    with open(os.path.join(save_dir, f"{name}_metrics.json"), "w") as file:
        json.dump(
            {
                ...
                "n_bins": n_bins,
                "count": int(N),
                "bin_accuracy": [float(v) for v in bin_acc],
                "bin_confidence": [float(v) for v in bin_conf],
            },
            file,
            indent=2,
        )
    print(f"Saved reliability diagram -> {save_dir}/{name}.png (ECE={ece:.3f}, MCE={mce:.3f})")
