...
def _parse_filename_bits(stem: str):
    m_rpm = re.search(r"rpm(\d+)", stem)
    m_render = re.search(r"(render[A-Za-z]+)", stem)
    rpm = int(m_rpm.group(1)) if m_rpm else None
    render = m_render.group(1) if m_render else None
    return rpm, render


def _to_base_name(names):
    if isinstance(names, (list, tuple)):
        name = names[0]
    else:
        name = names
    return osp.splitext(osp.basename(str(name)))[0]
...
if TEST_BOOL:
    ...
    with torch.no_grad():
        for frames, parameters, hotvector, names, rpm_idx, sample_info in tqdm(test_dl):
